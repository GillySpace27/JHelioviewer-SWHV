...
/**
 * The Grid palette's master toggle is the SAME switch as the sidebar row's checkbox, in both
 * directions, or the two would disagree about whether the grid is on.
 *
 ...
 *
 * <p>Run: java -cp "bin:extra/test-classes:resources:lib/*" org.helioviewer.jhv.gui.component.GridPaletteContentCheck
 */
public final class GridPaletteContentCheck {
    ...
    public static void main(String[] args) throws Exception {
        ...
        expect("there is a grid layer to bind to", layer != null);
        if (layer == null)
            System.exit(1);

        Component content = GridPaletteContent.build();
        ...
        // The settings stay live either way: the toggle is right there, and a grid can be set up
        // before it is shown.
        ...
        expect("and not greyed while the grid is off", options != null && options.isEnabled());

        layer.setEnabled(was);
        Layers.fireLayerUpdated(layer);

        System.out.println(failures == 0 ? "GridPaletteContentCheck: PASS" : "GridPaletteContentCheck: " + failures + " FAILURE(S)");
        if (failures != 0)
            System.exit(1);
    }

    private GridPaletteContentCheck() {}
