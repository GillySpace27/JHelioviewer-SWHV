...
/**
 * The Disk slider's mapping: logarithmic, continuous, and running the same way as its neighbours.
 *
 * <p>Two things go wrong quietly in a mapping shaped like this. The direction can invert, which
 * nobody notices until a slider "feels backwards" -- this one already did once, and the whole
 * palette has since been settled on right-is-bigger, so Disk must grow rightward like Warp, Crop
 * and Zoom. And a round trip can drift, so the handle jumps a pixel every time the palette is
 * rebuilt.
 *
...
public final class DiskSliderCheck {
    ...
    public static void main(String[] args) {
        ...
        expect(Math.abs(left - Display.DISK_SCALE_MIN) < 1e-9, "leftmost is the minimum, got " + left);
        expect(Math.abs(right - Display.DISK_SCALE_MAX) < 1e-9, "rightmost is the maximum, got " + right);
